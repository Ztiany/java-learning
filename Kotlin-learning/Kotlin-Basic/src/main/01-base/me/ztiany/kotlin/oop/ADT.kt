...
/**
 * 在计算机编程，特别是函数式编程与类型理论中，ADT是一种组合类型（composite type）。例如，一个类型由其他类型组合而成。
 * 两个常见的代数类型是：
 *
 * - “和”（sum）类型；
 * - “积”（product）类型。
 ...
 * - y * 3 = 21
 *
 * 上面表达式中的 x、y 就是代数。通过解上面的方程，我们知道了代数 x 代表的是数字1，而代数 y 代表的是数字 7。再仔细看一下上面的方程，我们还发现了两个操作符：“+”和“*”。
 * 那么通过代数和这些操作符能做什么呢？再看看两个表达式：
 *
 * - x * 1 = z
 * - a + 2 = c
 *
 *可以看出，第 1 个表达式中代数 x 与 1 通过乘法操作得到了一个新的代数 z，第 2 个表达式中代数 a 与 2 通过加法操作得到了一个新的代数 c。
 * 到这里，我们其实就可以思考一下了，如果把上面表达式中的代数与数字换成编程语言中的类型或者值，那它们之间通过某种操作是不是就可以得到某种新的类型呢？
 *
 * 而当我们将这些代数或者数字换成类型，那么这种被我们用代数或者数字换成的类型，以及通过这些类型所产生的新类型就叫作**代数数据类型（ADT）**。
 *
 * **那么 ADT 究竟有什么用呢**？其实 ADT 的应用很广，就拿我们所熟知的业务逻辑来说吧，我们可以将一些比较简单的类型通过某种“操作符”而抽象成比较复杂而且功能强大的类型。
 * 另外在编程语言中，某些常见的类型其实就是代数类型，比如枚举类。并且，ADT 是类型安全的，使用它可以为我们免去许多麻烦。
 *
 * ADT 中常见的两种类型：
 *
...
///////////////////////////////////////////////////////////////////////////
// 2. ADT在函数式编程中一个非常大的用处，就是结合模式匹配。虽然Kotlin并没有在极大程度上支持模式匹配，然而when表达式依旧是非常强大的一个语言特性。
///////////////////////////////////////////////////////////////////////////
/**
 ...
 * 2. 要想了解 Kotlin 首席设计师关于模式匹配的思考，可以参考他的这封邮件的内容：https://www.mail-archive.com/amber-spec-experts@openjdk.java.net/msg00006.html
 *
 * 模式匹配（Pattern Matching）：其实模式匹配与 Java 的正则匹配非常相似（Pattern 类），只是模式匹配中匹配的不仅有正则表达式，还可以有其他表达式。这里的“表达式”就是下面将要介绍的模式。
 *
...
// 以上 3 种就是在Kotlin中比较常见的模式。看了上面 3 种模式的匹配方式后，你可能会想，这就是模式匹配吗？和 if-else 有什么区别？的确，
// 上面的 3 种模式在进行匹配的时候，我们其实都可以用 if-else 或者 switch-case 来实现，就像我们在 Java 中经常做的那样。那么模式匹配的威力到底体现在哪里呢？

//查看 《Kotlin 核心编程》第四章中的 Scale 的模式匹配示例代码。
...
上面几种模式匹配都能用 if-else 或者 switch-case 语句来实现，那么有没有哪种模式是用 if-else 等语句实现起来有困难，但是用模式匹配却会很简洁的呢？答案是肯定的，那就是嵌套表达式。
 */
...
// 下面我们就利用上述结构来实现一些需求。在进行整数表达式计算的时候，你往往会遇到许多可以简化的表达式，比如“1+0”就可以简化为1，“1*0”就可以简化为0。
