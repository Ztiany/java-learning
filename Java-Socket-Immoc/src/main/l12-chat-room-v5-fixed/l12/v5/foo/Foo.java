package l12.v5.foo;

import java.io.File;
import java.io.IOException;
import java.util.UUID;

/**
 * @author Ztiany
 * Email ztiany3@gmail.com
 * Date 2018/11/24 11:39
 */
public class Foo {

    /* ------------------------------------- 桥接通信部分【带空格的表示需要命令参数】 ------------------------------------- */
    // 绑定 Stream 到一个命令链接(带参数)【服务端会启动两个链接到服务端，一个连接发送命令操作另一个链接】
    public static final String COMMAND_CONNECTOR_BIND = "--m c bind ";//+ id
    // 创建对话房间
    public static final String COMMAND_AUDIO_CREATE_ROOM = "--m a create";
    // 加入对话房间(带参数)
    public static final String COMMAND_AUDIO_JOIN_ROOM = "--m a join ";// + id
    // 主动离开对话房间
    public static final String COMMAND_AUDIO_LEAVE_ROOM = "--m a leave";

    // 回送服务器上的唯一标志(带参数)
    public static final String COMMAND_INFO_NAME = "--i server ";// + id
    // 回送语音群名(带参数)
    public static final String COMMAND_INFO_AUDIO_ROOM = "--i a room ";// + id
    // 回送语音开始(带参数)
    public static final String COMMAND_INFO_AUDIO_START = "--i a start ";// + id
    // 回送语音结束
    public static final String COMMAND_INFO_AUDIO_STOP = "--i a stop";
    // 回送语音操作错误
    public static final String COMMAND_INFO_AUDIO_ERROR = "--i a error";// + id

    /* ------------------------------------- 通用部分 ------------------------------------- */
    /**
     * 退出聊天
     */
    public static final String COMMAND_EXIT = "00bye00";

    /* ------------------------------------- 群操作部分 ------------------------------------- */
    /**
     * 加入群聊
     */
    public static final String COMMAND_GROUP_JOIN = "--m g join";

    /**
     * 退出群聊
     */
    public static final String COMMAND_GROUP_LEAVE = "--m g leave";

    /**
     * 默认群名
     */
    public static final String DEFAULT_GROUP_NAME = "God";

    public static File getCacheDir(String dir) {
        String path = System.getProperty("user.dir") + (File.separator + "cache" + File.separator + dir);
        File file = new File(path);
        if (!file.exists()) {
            if (!file.mkdirs()) {
                throw new RuntimeException("Create path error:" + path);
            }
        }
        return file;
    }

    public static File createRandomTemp(File parent) {
        String string = UUID.randomUUID().toString() + ".tmp";
        File file = new File(parent, string);
        try {
            boolean ignore = file.createNewFile();
        } catch (IOException e) {
            e.printStackTrace();
        }
        return file;
    }

}