...
import l9.v3.clink.core.Frame;
import l9.v3.clink.core.IoArgs;
import l9.v3.clink.core.ReceivePacket;
import l9.v3.clink.frame.AbsReceiveFrame;
import l9.v3.clink.frame.CancelReceiveFrame;
import l9.v3.clink.frame.ReceiveEntityFrame;
import l9.v3.clink.frame.ReceiveFrameFactory;
import l9.v3.clink.frame.ReceiveHeaderFrame;
...
class AsyncPacketWriter implements Closeable {
    ...
    synchronized IoArgs takeIoArgs() {
        //如果 frameTemp为null，则先解析帧的头六个字节，用于确定是什么类型的数据，用于接下来的帧的构建
        if (frameTemp == null) {
    ...
    synchronized void consumeIoArgs(IoArgs args) {
        ...
        if (frameTemp == null) {
            ...
            //处理情况1
            if (temp == null) {//temp == null 说明被取消了或者 args 消费完了，都直接返回
        ...
        do {
            try {
                //currentFrame.handle(args)用于从args中读取数据，只有frame被填满了，才会返回true。
                //currentFrame.handle(args) == true 说明当前currentFrame被填满了
                //handle 方法会直接将数据写入到 Channel 中
                if (currentFrame.handle(args)) {
                    ...
                    if (currentFrame instanceof ReceiveHeaderFrame) {
                        ...
                        //根据头帧信息创建一个包
                        ReceivePacket packet = mPacketProvider.takePacket(headerFrame.getPacketType(), headerFrame.getPacketLength(), headerFrame.getPacketHeaderInfo());
